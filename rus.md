...
Возможно, вы даже не осознаёте, но LESS и SASS сильно связывают руки. Я решил
избавиться от этого написав свой CSS-препроцессор. Отказался от CSS
и перенес все в мир JavaScript. Эта статья об [AbsurdJS][1]: небольшом Node.js
...
Если вы часто пишете CSS, то наверняка используете препроцессоры. Популярных 
всего два: LESS и SASS. Оба берут что-то, похожее на CSS, творят над этим
магию и выдают в итоге нормальный, чистый CSS. Я, всего-навсего, заменил
инструмент и формат исходных данных. Я не стал изобретать новые языки и
...
несколько минусов. Вам придётся оборачивать некоторые свойства в кавычки, как и
все значения. Это будет занимать больше времени, чем обычно, но, как вы увидите
позднее, оно того стоит.
...
А так выглядит его эквивалент на AbsurdJS. Это простой Node.js модуль:
...
В SASS есть возможность использовать *плейсхолдеры*, которые делают то же самое.
...
AbsurdJS заботится об этом сам и выдаёт только одно определение. На входе код
может выглядеть так:
...
начнёте использовать их для написания вещей посложнее, вы застрянете.
...
эти проблемы разом, всего лишь работая с JavaScript.
...
[AbsurdJS][5] очень прост, но позволяет обойтись без популярных CSS-препроцессоров. 
В нём можно использовать вложенные селекторы, всплытие медиавыражений, импорт файлов, 
переменных, миксинов. В то же время он привносит больше гибкости, потому что это 
чистый JavaScript. Он даже [работает с GruntJS][6]. Я бы хотел получить обратную связь 
и буду рад, если вы примете участие в проекте. Официальный репозиторий находится
