...
изменить это тем, что написал свой CSS препроцессор. Я отказался от CSS и
перенес все в мир JavaScript. Эта статья о [AbsurdJS][1]: небольшом Node.js
...
Вы должны определить функцию `module.exports`. Она принимает параметорм ссылку
на API, который содержит несколько методов, но самый важный из них - `add`.
Просто передайте в метод JSON-объект и он будет сконвертирован в CSS.
...
результат радует далеко не всегда.
...
парсит правила и объединяет их. Давайте рассмотрим пример посложнее:
...
браузеры понастоящему быстры и это не самая важная оптимизация, но так можно
...
Хорошо. У нас есть примеси, переменные, расширения, функции, но как только вы
...
нельзя использовать примесь, объявленную в другой примеси. Возможна причина в
...
свойство `theme` и пропишем ему возможные значения.
...
CSS препроцессоров. Он все равно сохранил некоторые фичи, такие как вложенные
селекоторы, всплытие медиавыражений, импорт файлов, переменные, примеси и
другие. В то же время он привносит больше гибкости, потому что это чистый
JavaScript. Он даже [работает с GruntJS][6]. Я был бы хотел получить обратную
связь и был бы рад, если вы примете участие в проекте. Официальный репозиторий
находится здесь [https://github.com/krasimir/absurd][7].
